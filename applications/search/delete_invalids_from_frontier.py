...
from datamodel.search.Adityan1MonishppSkanade1_datamodel import Adityan1MonishppSkanade1Link
...
@Deleter(Adityan1MonishppSkanade1Link)
@Getter(Adityan1MonishppSkanade1Link)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(Adityan1MonishppSkanade1Link)
        print "Found ", len(ls), " links."
        count = 0
        for l in ls:
            if not is_valid(l.full_url):
                self.frame.delete(Adityan1MonishppSkanade1Link, l)
                count += 1
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_Adityan1MonishppSkanade1".format(CrawlerFrame.app_id),
